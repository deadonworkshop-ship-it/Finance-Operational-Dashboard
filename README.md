...
+
+### What makes it a “legitimate iPhone app”
+- You package this web app inside a native iOS shell using **Capacitor**.
+- You open/build/sign it in **Xcode** with your Apple Developer account.
+- You can install it on your own phone (personal use), and optionally submit to the App Store later.
+
+### Prerequisites (Mac required for iOS build)
+- macOS + Xcode (latest stable)
+- Apple ID (free for personal device testing, paid Developer Program for TestFlight/App Store)
+- Node.js 18+
+
+### Step-by-step: convert this repo to iPhone app
+From the project root:
+
+```bash
+npm init -y
+npm install @capacitor/core @capacitor/cli
+npx cap init "Finance Operational Dashboard" "com.yourname.financeops" --web-dir .
+```
+
...
+```
+
+### Xcode setup for device install
+1. Select the `App` target.
+2. Set **Bundle Identifier** (unique, e.g., `com.yourname.financeops`).
+3. In **Signing & Capabilities**, choose your Team.
+4. Connect your iPhone and select it as run target.
+5. Press **Run** in Xcode.
+
+### Updating app content after web changes
+Any time you change `index.html` / assets in this repo:
+
+```bash
+npx cap sync ios
+npx cap open ios
...
+
+### Personal install vs App Store
+- **Personal phone install only**: free Apple ID can work for local device deployment.
+- **TestFlight/App Store distribution**: requires paid Apple Developer Program and App Store review.
+
+### Recommended next step for your app
+Because your app stores data in local browser storage right now, consider moving planner data to a native-safe storage layer before App Store submission (for reliability/backups), e.g. Capacitor Preferences/SQLite + optional iCloud sync.
+
+
